...
class Object:
    _subclasses: ClassVar[dict[str, type[Object]]] = {}
    ...
    def __new__(cls, json):
        return super().__new__(
            Object._subclasses[json["object"]] if cls is Object else cls
        )

    def __init__(self, json):
        if json["object"] != self.object:
            raise Exception(
                f"Creating {type(self)} from json object {json.get('object')!r}"
            )
        self._json = json
    ...
    def _submap(json):
        return (
            Object(json)
            if isinstance(json, dict) and "object" in json
            else Object._map(json)
        )
...
class Card(Object, object="card"):
    ...
    def front(self):
        return (
            self.card_faces[0]
            if "card_faces" in self._json and self.layout != "split"
            else self
        )
